...
int main(int argc, char* argv[])
{
  ...
  unsigned char* buffer = malloc(yres * xres * 6);
  ...
  //        fwrite (black, 6, 1, fp);
  	    ...
  	    int boffset = j * xres * 6 + i * 6;
	    memcpy(buffer + boffset, black, 6);
        }
        else {
          /* exterior */
          ...
          color[5] = k & 255;
//        fwrite(color, 6, 1, fp);
	  int coffset = j * xres * 6 + i * 6;
	  memcpy(buffer + coffset, color, 6);
        };
      }
    }
  }
